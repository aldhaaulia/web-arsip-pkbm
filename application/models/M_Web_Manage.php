...
class M_Web_Manage extends CI_Model {
	...
	function slider_fasilitas_getByid($id) {
		$is_success = false;
		$new_data = array();

		$this->db->select('a.id, a.title, a.subtitle, a.`desc`, a.image');
		$this->db->from($this->table_slider_fasilitas . ' as a');
		$this->db->where('a.id', $id);
		$this->db->limit(1);
		$get = $this->db->get();
	...
	function slider_fasilitas_list() {
		...
		# set variable
		$select    = 'a.*, a.is_active as status';
		$tbl_query = $this->table_slider_fasilitas . ' as a';
		$joinTable = array();
		$joinCond  = array();
		$column_search = array();
		$column_order  = array();
	...
	function slider_fasilitas_add($dataPost) {
		$is_success = false;

		if (isset($_FILES['file_image'])) {
			$nameShort = 'slider_fasilitas';
			$file_path = $this->path_upload_file . '/slider_fasilitas/';
			$config_upload = array(
				'upload_path' => $file_path
			);
		...
		$this->db->insert($this->table_slider_fasilitas, $dataPost);
	...
	function slider_fasilitas_update($dataPost, $where) {
		...
		$this->db->update($this->table_slider_fasilitas, $dataPost, $where);
	...
	function berkas_update($dataPost) {
		...
		if (isset($_FILES['file_berkas'])) {
			$nameShort = 'register_';
			$file_path = $this->path_upload_file . '/register/';
			$config_upload = array(
				'upload_path' => $file_path
			);

			$uploadData = $this->uploadsData('file_berkas', $config_upload, $nameShort);

			$res_uploadData = $uploadData['data'];

			if ($uploadData['success']) {
				$where = array('name' => 'berkas_file_register');
				$dataSave['value'] = $res_uploadData['filename'];
				$dataSave['tgl_update'] = $tgl_now;
	...
	function web_update($dataPost) {
		...
		if (isset($dataPost['is_register_open'])) {
			$where = array('name' => 'is_register_login_open');
			$dataSave['value'] = $dataPost['is_register_open'];
			$dataSave['tgl_update'] = $tgl_now;
