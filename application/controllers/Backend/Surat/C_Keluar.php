...
class C_Keluar extends MY_Controller
{
	...
	function index()
	{
		...
		$get_settings = $this->my_model->get_setting();
		$data['setting'] = $get_settings;
		$data['surat'] = $this->m_surat->getSurat('2');
		$data['disposisi'] = $this->m_surat->getSuratDisposisi('2');
		$data['all'] = $this->m_surat->getDokBiasa('2');


		$this->load->view('backend/surat/keluar/v_surat_keluar', $data);
	}
	...
	function add()
	{
		...
		$this->load->view('backend/surat/keluar/v_surat_keluar_form', $data);
	}

	function savesurat()
	{
		$save = $this->m_surat->save('2');
		if ($save) {
			$data['success'] = true;
			$data['msg'] = "Simpan Data Berhasil";
		} else {
	...
	public function deleteMulti()
	{
		...
		$delete = $this->m_surat->multiDelete1($from_date, $to_date, 2);
		if ($delete) {
			$data['success'] = true;
