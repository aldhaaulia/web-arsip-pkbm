...
class C_Web_Manage extends MY_Controller {
	...
	function slider_fasilitas() {
		$get_settings = $this->my_model->get_setting();
		$data['setting'] = $get_settings;

		$this->load->view("backend/web_manage/frontend/v_slider_fasilitas", $data);
	}

	function slider_fasilitas_list() {
		$this->m_web_manage->slider_fasilitas_list();
	}

	function slider_fasilitas_getByid() {
		$id = $this->input->post('id');
		return $this->m_web_manage->slider_fasilitas_getByid($id);
	}

	function slider_fasilitas_save() {
		$tgl_now = date('Y-m-d H:i:s');
		...
		if (!empty($id)) {
			$where = array('id' => $id);
			$dataSave['tgl_update'] = $tgl_now;
			$save = $this->m_web_manage->slider_fasilitas_update($dataSave, $where);
		} else {
			$dataSave['tgl_create'] = $tgl_now;
			$save = $this->m_web_manage->slider_fasilitas_add($dataSave);
		}
	...
	function slider_fasilitas_update() {
		$r = array();
		$dataSave = array();
		$tgl_now = date('Y-m-d H:i:s');
	...
	function web_save() {
		...
		$input_school_register = $this->input->post('input_school_register');

		if($input_school_register == 1){
			$input_school_register = "true";
		}else{
			$input_school_register = "false";
		}
		...
		$dataSave = array(
			...
			'is_register_open' => $input_school_register,
		);
